...
/* форма для добавления события при клике на ячейку календаря */

var calendarCell = $(".cell");

function eventWindowOnClick () {
	var windowOnClick = $("<div class='windowOnClick'>");
	var form4CreateEvent = $("<form class='form4CreateEvent'>");
	var lblEventType = $("<label class='lbl' for='event_type'>");
	var input4EventType = $("<input id='event_type' name='event_type'>");
	var lblEventDate = $("<label class='lbl' for='event_date'>");
	var input4EventDate = $("<input id='event_date' name='event_date'>");
	var lblEventуDescr = $("<label class='lbl' for='event_descr'>");
	var input4EventDescr = $("<input id='event_descr' name='event_descr'>");
	var table = $(".table");
	$(form4CreateEvent).append(lblEventType);
	$(form4CreateEvent).append(input4EventType);
	$(form4CreateEvent).append(lblEventDate);
	$(form4CreateEvent).append(input4EventDate);
	$(form4CreateEvent).append(lblEventуDescr);
	$(form4CreateEvent).append(input4EventDescr);
	$(windowOnClick).append(form4CreateEvent);
	$(table).append(windowOnClick);
}


$(calendarCell).on("click", function () {
	$(".windowOnClick").toggleClass("windowOnClickToggle");
});